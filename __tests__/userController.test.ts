...
describe('UserController', () => {
  ...
  //   (userService.assignGroup as jest.Mock).mockImplementation(() => {
  //     throw undefined;
  //   });
  //   const req = httpMocks.createRequest({
  //     method: 'PUT',
  ...
  it('should handle non-Error thrown in completeProfile', async () => {
    jest
      .spyOn(require('../src/service/authService').default, 'completeProfile')
      .mockImplementation(() => {
        throw false;
      });
    const req = httpMocks.createRequest({
  ...
  it('should handle non-Error thrown in loginLocal', async () => {
    jest
      .spyOn(require('../src/service/authService').default, 'loginLocal')
      .mockImplementation(() => {
        throw 404;
      });
    const req = httpMocks.createRequest({
  ...
  it('should handle non-Error thrown in loginGoogleCallback', async () => {
    jest
      .spyOn(require('../src/service/authService').default, 'generateToken')
      .mockImplementation(() => {
        throw 500;
      });
  ...
  it('should return 400 if createUser missing required fields', async () => {
    const req = httpMocks.createRequest({
      method: 'POST',
      body: { email: 'test@example.com', password: 'pass', phoneNumber: '0700000000' },
    });
    const res = httpMocks.createResponse();
    await userController.createUser(req, res);
    expect(res.statusCode).toBe(400);
